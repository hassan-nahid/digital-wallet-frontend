...
const SendMoney = () => {
  ...
  const { data: userData, isFetching, refetch } = useGetUserByEmailQuery(searchEmail, { skip: !searchEmail });
    ...
    <div className="flex justify-center items-center min-h-[60vh] bg-background">
      <div className="w-full max-w-md bg-white dark:bg-muted rounded-2xl shadow-xl p-0 border border-primary/20">
        ...
        <div className="bg-primary/90 rounded-t-2xl px-8 py-5 flex flex-col items-center gap-1">
          <h2 className="text-2xl font-bold text-white tracking-wide">Send Money</h2>
          <p className="text-xs font-semibold text-primary-foreground/80">Search and select a user to send money</p>
        </div>
        <div className="p-6 flex flex-col gap-6">
          ...
          {!selectedUser && userData?.data && (
            <div className="mb-2 p-4 rounded-xl bg-primary/5 border border-primary/20 flex flex-col gap-1 shadow-sm">
              ...
              <Button type="button" size="sm" className="mt-2 w-fit" onClick={handleSelectUser} variant="outline">Select User</Button>
            </div>
          )}
          {/* Show selected user */}
          {selectedUser && (
